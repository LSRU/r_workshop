...
+ **manipulating** data. Without entering into details, everything like merging, filtering, sorting, aggregating etc.
    - `dplyr`
+ **plotting** data. _R_ does a amazing job
    - `ggplot2`
+ **chaining** functions. Sick of parenthesis, we will move to the pipe operator `%>%` 

4 other aspects won't be covered, or could be depending on the interest:
...
+ **versionning**, using `git`. Not really specific de to _R_ but nicely integrated to RStudio.   
...
Of course, everything will be done using [Rstudio ](https://www.rstudio.com/products/RStudio/#Desktop) that makes you life way easier.
...
A common question is _how do I merge 2 lists based on this/those id(s)?_

Obviously, they are not sorted the same way, that is super easy in R, see this toy example
```{r}
library("dplyr")
a <- data.frame(a = 1:3,
                name = c("a", "a", "b"))
b <- data.frame(a = c(2, 3, 1),
                other = c("yes", "no", "yes"))
a
b
inner_join(a, b)
...
Tyding data is one observation per row, one variable per column.
...
See one sample per column, and we cannot apply which are control or treated
```{r}
gene_expression
```

this is the _wide_ format, we need the _long_ format.  
Now we can set that the first 3 are controls.
```{r}
library("tidyr")
...
Using the previous summarised example. Since the data is **tidy**, we can use it directly.
See that the legend is automatically added
