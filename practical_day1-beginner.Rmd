...
your `data` folder.
- read it using `read_tsv()` in an data frame named `gse`. The file will be uncompressed transparently.
```{r, solution=TRUE}
...
```
- Correct the mistake by reading again the file with relevant option in a new object.
- Replace the wrong column in `gse` by the correct one in the new data frame.
```{r, solution=TRUE}
