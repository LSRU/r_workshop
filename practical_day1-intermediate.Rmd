...
Here, we will investigate the relationship between the expression of _ENTPD5_ and mir-182 as described by the authors. Even if, the data are normalised and should be ready to use, quite 
an extensive amount of work is required to reproduce the claimed result.
...
linear models output by `lm()` are lists. Since `data.frame` are much easier to work with,
[David Robinson](http://varianceexplained.org/) developed [`broom`](https://cran.r-project.org/web/packages/broom/index.html).
More will be done in the advanced lecture, but here see test the `broom::tidy()` coupled with `dplyr::do()`.
```{r}
library("broom")
expression %>%
  separate(source_name_ch1, c("source", "rest"), sep = 12) %>%
...
The estimate of the intercept is not meaningful so filtered out.
...
- Count how many `hsa-mir` not star are in present on the array GPL8786
```{r, solution=TRUE}
fData(gse35834[[2]]) %>%
  filter(grepl("^hsa", ID)) %>%
