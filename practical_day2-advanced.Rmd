...
Now, we want to see how many of 'linear-trend' genes are overlapping to the nutrient starvations

Venn Diagrams are an option but we have 5 sets and intersections are just impossible to see.
The alternative is the [upset](http://vcg.github.io/upset/) for which a `R` [implementation](https://github.com/hms-dbmi/UpSetR)
was done.

The object needed is a `data.frame` with:
...
- Starting from `cleaned_lm`, filter out the models with $r^2 < 0.9$ and
 empty systematic name (_i.e_ = "").

- select only the `systematic_name`, `nutrient` columns only 
- add a column `set` that contains 1 for all rows.
- convert from long to wide format `nutrient` fill up by `set`.
...
- Move `the systematic` from column to rownames (`tibble::column_to_rownames()`)
Save as `mat_upset`
- The pseudo matrix is almost done, we have absence as `NA` and presence as `1`.
- Replace all `NA` by 0
- set the class of `mat_upset` to `data.frame`, `upset` does not handle `tbl_df`
