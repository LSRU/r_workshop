...
and the migration of your installed packages whenever a major `R` release happens. Recently, a package can [help you](#reinstallr) with this.
...
A very recent package `reinstall` is doing a neat job looking at the installed packages and those you already used by parsing your `.R`, `.Rmd` etc. your R files.

For example, after I upgraded to `3.3` I forgot some packages and can decide if I want to install or not.
```{r, eval=FALSE}
